...
function AdminSidebar({ mobileOpen, handleDrawerToggle, desktopCollapsed, setDesktopCollapsed }) {
  ...
  const drawerContent = (
    <>
      <Toolbar sx={{ mb: 1, mt: 1, display: 'flex', justifyContent: 'space-between', pr: 1 }}>
  ...
  return (
    <Box 
      component="nav" 
      sx={{ 
        width: { sm: desktopCollapsed ? 0 : drawerWidth }, 
        flexShrink: { sm: 0 },
    ...
    >
      ...
      <Drawer
        variant="permanent"
        sx={{
          display: { xs: 'none', sm: desktopCollapsed ? 'none' : 'block' },
          '& .MuiDrawer-paper': { 
            boxSizing: 'border-box', 
            width: drawerWidth,
            bgcolor: '#0F172A', // Dark navy
