...
function ViewTemplates() {
  ...
  const computeFieldStats = (schemaArray) => {
    ...
    const optional = inputFields - required;
    const typeBreakdown = Object.entries(typeCount).map(([type, count]) => `${count} ${type}`).join(', ');
    
    return { inputFields, headings, sections, required, optional, typeBreakdown };
  };
          ...
          {fieldStats && (
            <Box sx={{ mb: 3, p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
              ...
              <Typography variant="body2">
                <strong>Input Fields:</strong> {fieldStats.inputFields} 
                ({fieldStats.required} required, {fieldStats.optional} optional)
                {fieldStats.headings > 0 && ` — Headings: ${fieldStats.headings}`}
                {fieldStats.sections > 0 && ` — Sections: ${fieldStats.sections}`}
              </Typography>
              <Typography variant="body2">
                <strong>Types:</strong> {fieldStats.typeBreakdown || 'None'}
              </Typography>
            </Box>
          )}
