...
import { getAdminSubmissions } from '../../services/adminService';
...
function Submissions() {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      ...
      try {
        ...
        // Since backend might not have status for submissions yet, we simulate it for demonstration if missing
        const augmentedData = data.map((sub, index) => ({
          ...sub,
          status: sub.status || (index % 3 === 0 ? 'APPROVED' : index % 5 === 0 ? 'REJECTED' : 'PENDING')
        }));
        setSubmissions(augmentedData);
      } catch (err) {
  ...
  const handleBulkAction = (action) => {
    if (window.confirm(`Are you sure you want to ${action} ${selectedIds.length} submissions?`)) {
      // Execute API call here, currently mocking the UI state change
      ...
      {selectedIds.length > 0 && (
        <Paper sx={{ p: 1.5, mb: 3, borderRadius: 2, bgcolor: '#EFF6FF', border: '1px solid #BFDBFE', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Stack direction="row" spacing={1}>
            ...
            <Button size="small" variant="outlined" startIcon={<DownloadIcon />} sx={{ textTransform: 'none', bgcolor: '#fff' }}>
              Export
            </Button>
