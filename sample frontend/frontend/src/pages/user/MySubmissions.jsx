...
function MySubmissions() {
  ...
  const getActiveStep = (status) => {
    ...
    if (s === 'CORRECTION_REQUIRED') return 1;
        ...
        <DialogContent dividers sx={{ p: 0 }}>
          ...
          <Box sx={{ p: 4, bgcolor: 'slate.50' }}>
            ...
            <Stepper activeStep={getActiveStep(selectedSubmission?.status)} alternativeLabel>
              {STEPS.map((label) => (
                <Step key={label}>
                  <StepLabel error={label === 'Under Verification' && selectedSubmission?.status === 'Correction Required'}>
                    {label}
                  </StepLabel>
                </Step>
              ))}
            ...
            {/* Correction UI Mock */}
            {selectedSubmission?.status?.toUpperCase() === 'CORRECTION_REQUIRED' && (
              <Alert severity="warning" sx={{ mt: 4, borderRadius: 2 }}>
                <Typography variant="subtitle2" gutterBottom sx={{ fontWeight: 700 }}>Correction Required</Typography>
